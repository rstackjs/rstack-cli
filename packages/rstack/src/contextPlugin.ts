...
const createContextPlugin = (loaded: LoadedRstackConfig, cwd: string): RstackPlugin => ({
  name: 'rstack:context',
  async setup(api) {
    const configFilePath = loaded.filePath === null ? null : await realpath(loaded.filePath);
