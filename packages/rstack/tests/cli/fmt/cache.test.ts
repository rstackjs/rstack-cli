...
test.each([
  ...
  expectSingleCleanEntry(readFmtCache('.rstack/cache/fmt/v2.json'), 'index.ts');
...
test.each(['relative', 'absolute'] as const)('uses a %s custom cache location', (kind) => {
  ...
  expect(result.status).toBe(0);
  expectSingleCleanEntry(readFmtCache('custom-cache/v2.json'), 'index.ts');
...
test('uses an explicit config root cache from a subdirectory', () => {
  ...
  expect(projectFileExists('.rstack/cache/fmt/v2.json')).toBe(true);
  expect(projectFileExists('packages/app/.rstack')).toBe(false);
  expectSingleCleanEntry(readFmtCache('.rstack/cache/fmt/v2.json'), 'packages/app/index.ts');
});

test('recovers from a corrupted cache', () => {
  writeProjectFile('index.ts', 'const value = 1;\n');
  const first = runFmt(['--check', 'index.ts']);
  writeProjectFile('.rstack/cache/fmt/v2.json', '{');
  ...
  expect(JSON.parse(readProjectFile('.rstack/cache/fmt/v2.json'))).toMatchObject({ version: 2 });
});
