...
| ------------------------ | ------------------------------------------------- | --------------------------------------- |
| `rstack`                 | Rstack CLI configuration and plugin APIs          | Configure and extend Rstack CLI         |
...
Import `define` from `rstack` to register tool configurations and plugins in `rstack.config.ts`; see [Configuration APIs](./configuration#configuration-apis) and [Plugins](./plugins) for details.

The main entry point also exports the public plugin types, including `RstackPlugin`, `RstackPluginAPI`, `RstackPluginContext`, `RstackConfigMap`, `FmtConfig`, and `StagedConfig`.
