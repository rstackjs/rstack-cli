---
description: 'Run lint-staged tasks against Git-staged files with the rs staged command.'
---
...
A common pattern is to pair `rs staged` with [`rs hooks`](./hooks) and run staged-file tasks from a `pre-commit` hook.
