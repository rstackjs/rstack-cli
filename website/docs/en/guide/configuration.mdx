...
> [`rs mcp`](./cli/mcp) is a read-only store reader. It resolves the checkout from its launch directory and does not load or execute `rstack.config.*`; enable capture on the producer commands, [`rs dev`](./cli/dev), [`rs build`](./cli/build), and [`rs lib`](./cli/lib), instead.
...
Use [`rs mcp`](./cli/mcp) to read the completed observations through the local read-only MCP server.
