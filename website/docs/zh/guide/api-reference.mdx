...
| 导入路径                 | 内容                                      | 使用场景                 |
| ------------------------ | ----------------------------------------- | ------------------------ |
| `rstack`                 | Rstack CLI 配置与插件 API                 | 配置并扩展 Rstack CLI    |
...
从 `rstack` 导入 `define`，用于在 `rstack.config.ts` 中注册各项工具配置和插件；详细用法请参阅[配置 API](./configuration#configuration-apis)和[插件](./plugins)。

主入口还会导出公开插件类型，包括 `RstackPlugin`、`RstackPluginAPI`、`RstackPluginContext`、`RstackConfigMap`、`FmtConfig` 和 `StagedConfig`。
