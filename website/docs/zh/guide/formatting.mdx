...
Rstack CLI 提供了基于 [Prettier](https://prettier.io/) 的格式化工具。相比直接使用 Prettier，`rs fmt` 提供了以下能力：

- **并行格式化**：通过 worker 池并行格式化文件。
- **SWC Next 解析器**：默认使用高性能的 [SWC Next](https://github.com/swc-project/swc-next) 解析器处理 JavaScript、JSX 和 TypeScript 文件。
