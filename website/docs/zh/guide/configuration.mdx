...
> [`rs mcp`](./cli/mcp) 是只读的存储读取器。它根据启动所在目录解析检出目录，不会加载或执行 `rstack.config.*`；请改为在生产者命令 [`rs dev`](./cli/dev)、[`rs build`](./cli/build) 和 [`rs lib`](./cli/lib) 上启用捕获。
...
使用 [`rs mcp`](./cli/mcp) 通过本地只读 MCP 服务器读取已完成的观测结果。
